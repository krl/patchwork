    ...
    return <div id="bookmarks">
      ...
      <SimpleInfinite onInfiniteLoad={this.onInfiniteLoad.bind(this)} infiniteLoadBeginBottomOffset={shouldLoadMore ? 100 : 0}>
        ...
        <MsgList
          ...
          emptyMsg="No bookmarked msgs found" />
      </SimpleInfinite>
    </div>
  }
}
...
function mapStateToProps (state) {
  const msgList = state.msgLists.Bookmarks
  const settings = state.views.Bookmarks.settings
  return {
    settings: settings,
...
function mapDispatchToProps (dispatch) {
  return {
    onLoad: () => dispatch(msglistCreate('Bookmarks', {
      fetchFn: app.ssb.patchwork.createBookmarkStream,
      cursorFn: (msg) => [msg.value.timestamp, msg.value.author],
      liveOptsFn: () => { gt: [Date.now(), null] },
      numInitialLoad: PAGE_SIZE
    })),
    onLoadMore: () => dispatch(msglistLoadMore('Bookmarks', PAGE_SIZE)),
    onOpenComposer: () => dispatch(viewOpen('composer')),
    onSelectFilter: (filter) => dispatch(viewUpdateSetting('Bookmarks', 'activeFilter', filter))
  }
}
